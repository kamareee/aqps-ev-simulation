...
class Vehicle:
    """
    Represents an EV for the AQPS Scheduler (Layer 1 & 2).
    Compatible with new queue_manager.py and scheduler.py.
    """

    id: str
    arrival_time_idx: int
    departure_time_idx: int
    target_energy_kwh: float
    max_charging_power_kw: float
    is_priority: bool = False
    initial_energy_kwh: float = 0.0
    energy_delivered: float = 0.0
    energy_needed: float = field(init=False)

    def __post_init__(self):
        self.energy_needed = max(0.0, self.target_energy_kwh - self.initial_energy_kwh)
...
class SessionInfo:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"SessionInfo({priority_tag} {self.session_id}, "
...
class PriorityQueueEntry:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"QueueEntry({priority_tag}, {self.session.session_id}, "
...
class PreemptionEvent:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"PreemptionEvent(t={self.timestamp}, {status}, "
...
class ThresholdViolationEvent:
    ...
    def severity(self) -> str:
        ...
        ratio = (
            self.capacity_shortfall / self.capacity_available
    ...
    def __repr__(self) -> str:
        return (
            f"ThresholdViolation(t={self.timestamp}, {self.severity.upper()}, "
...
class TOUDeferralEvent:
    ...
    def __repr__(self) -> str:
        return (
            f"TOUDeferralEvent(t={self.timestamp}, {self.session_id}, "
...
class TOUMetrics:
    ...
    def __repr__(self) -> str:
        return (
            f"TOUMetrics(t={self.timestamp}, {self.current_tou_period}, "
...
class QuantizationEvent:
    ...
    def __repr__(self) -> str:
        direction = (
            "↑" if self.rate_change > 0 else "↓" if self.rate_change < 0 else "="
        )
        return (
            f"QuantizationEvent({self.session_id}, "
...
class QuantizationMetrics:
    ...
    def quantization_efficiency(self) -> float:
        ...
        if self.pre_quantization_capacity <= 0:
            return 100.0
        return (
            self.post_quantization_capacity / self.pre_quantization_capacity
        ) * 100.0
    ...
    def __repr__(self) -> str:
        return (
            f"QuantizationMetrics(t={self.timestamp}, "
...
class ComputationalMetrics:
    ...
    def __repr__(self) -> str:
        return (
            f"ComputationalMetrics(t={self.timestamp}, "
...
class Phase4Config:
    ...
    pilot_signals: List[float] = field(
        default_factory=lambda: J1772_PILOT_SIGNALS.copy()
    )
...
class SimulationSummary:
    ...
    def __repr__(self) -> str:
        return (
            f"SimulationSummary(timesteps={self.total_timesteps}, "
